...
export const JobOverview: React.FC<JobOverviewProps> = ({
  ...
  return (
    <div className="p-6 bg-white rounded-xl border border-neutral-200 shadow-2xs">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          <div className="flex flex-wrap items-center gap-3">
            <span className="font-mono font-bold text-2xl text-neutral-900">{job.id}</span>
        ...
        {canManage && (
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2.5 p-3 bg-neutral-50 rounded-lg border border-neutral-200">
            ...
            <div className="relative">
              <select
                ...
                className="px-3 py-1.5 text-xs font-semibold bg-white border border-neutral-300 rounded-md text-neutral-900 focus:ring-2 focus:ring-neutral-900 focus:outline-none"
              >
                {statuses.map((s) => (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-5 pt-4 border-t border-neutral-100 text-xs">
        <div>
          <span className="text-neutral-400 uppercase text-[10px] block">Service Advisor</span>
          <span className="font-semibold text-neutral-800">{job.serviceAdvisor}</span>
        </div>
        <div>
        ...
        <div>
          ...
          <span className="text-neutral-800 font-medium">{formatDate(job.expectedCompletionDate)}</span>
        </div>
        <div>
