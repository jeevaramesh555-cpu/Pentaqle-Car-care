...
import { formatDate, formatOdometer, formatDateTime } from '../../utils/formatters';
...
export const DashboardPage: React.FC = () => {
  ...
  const loadData = async () => {
    const [j, v, c, recs, logs] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner / Welcome */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 bg-white rounded-xl border border-neutral-200 shadow-2xs">
        <div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            className="px-3.5 py-2 text-xs font-semibold text-neutral-700 bg-neutral-100 hover:bg-neutral-200 rounded-md transition-colors"
          >
            Browse All Vehicles
          </button>
          <button
            onClick={() => navigate('/jobs')}
            className="px-3.5 py-2 text-xs font-semibold text-white bg-neutral-900 hover:bg-neutral-800 rounded-md transition-colors shadow-2xs flex items-center gap-1.5"
          >
            <ClipboardList className="w-3.5 h-3.5" />
            <span>Job Card Board</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {pendingRecs.slice(0, 3).map((rec) => {
                ...
                return (
                  <div
                    ...
                    className="p-3 bg-neutral-50 hover:bg-neutral-100/80 rounded-lg border border-neutral-200/80 transition-colors cursor-pointer flex items-start justify-between gap-3 text-xs"
                  >
                    <div>
        ...
        <div className="space-y-6">
          ...
          {/* Workshop Activity / Audit Stream */}
          <div className="bg-white rounded-xl border border-neutral-200 shadow-2xs p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-neutral-500">
            ...
            <div className="space-y-3">
              {auditLogs.map((log) => (
                <div key={log.id} className="text-xs border-l-2 border-neutral-300 pl-3 py-0.5">
                  <div className="flex items-center justify-between text-[11px] text-neutral-400">
                    <span className="font-semibold text-neutral-700">{log.userName}</span>
                    <span>{formatDateTime(log.timestamp).split(',')[1]}</span>
                  </div>
                  <p className="text-neutral-600 mt-0.5 text-[11px] leading-tight line-clamp-2">
                    {log.description}
                  </p>
                </div>
              ))}
